...
namespace Interpreter.Compiler
{
    internal class Compiler
    {
        ...
        {
            readLibraryModule(Resources.c_runtimelibrary);
            readLibraryModule(Resources.c_Assert);
            readLibraryModule(Resources.c_Buttons);
            readLibraryModule(Resources.c_Byte);
            readLibraryModule(Resources.c_EV3);
            readLibraryModule(Resources.c_EV3File);
            readLibraryModule(Resources.c_LCD);
            readLibraryModule(Resources.c_Mailbox);
            readLibraryModule(Resources.c_Math);
            readLibraryModule(Resources.c_Motor);
            readLibraryModule(Resources.c_Program);
            readLibraryModule(Resources.c_Sensor);
            readLibraryModule(Resources.c_Speaker);
            readLibraryModule(Resources.c_Text);
            readLibraryModule(Resources.c_Thread);
            readLibraryModule(Resources.c_Vector);
            readLibraryModule(Resources.c_Sensor1);
            readLibraryModule(Resources.c_Sensor2);
            readLibraryModule(Resources.c_Sensor3);
            readLibraryModule(Resources.c_Sensor4);
            readLibraryModule(Resources.c_MotorA);
            readLibraryModule(Resources.c_MotorB);
            readLibraryModule(Resources.c_MotorC);
            readLibraryModule(Resources.c_MotorD);
            readLibraryModule(Resources.c_MotorAB);
            readLibraryModule(Resources.c_MotorAC);
            readLibraryModule(Resources.c_MotorAD);
            readLibraryModule(Resources.c_MotorBC);
            readLibraryModule(Resources.c_MotorBD);
            readLibraryModule(Resources.c_MotorCD);
            readLibraryModule(Resources.c_Row);
            readLibraryModule(Resources.c_Time);
        }
        ...
        internal void Start(Stream source, Stream targetstream, List<string> errorlist)
        {
        ...
        internal string CreateNativeCodeDownload()
        {
            StringBuilder b = new StringBuilder();
            byte[] c = ToByteArray(Resources.c_NativeCode);
        ...
        internal void memorize_reference(string name)
        {
        ...
        private void compile_atomic_statement(TextWriter target)
        {
            ...
            //Console.WriteLine(id + " " + variables.Count);
            s.GetSym();
        ...
        private void compile_variable_assignment(TextWriter target)
        {
            ...
            if (!variables.ContainsKey(varname))
            {
                ...
                //===
                //Console.WriteLine(variables[varname].ToString() + " " + varname);
                //===
            }
            else if (variables[varname] != e.type)
            {
                //===
                //Console.WriteLine("exception ===>" + variables[varname].ToString() + " " + varname + " " + e.type.ToString() + " " + e.PreparedValue());
                //===
                s.ThrowParseError("Can not assign different types to " + basicvarname);
                /*
                if (variables.ContainsKey(e.PreparedValue()))
                {
                    variables[e.PreparedValue()] = variables[varname];
                }
                */
            }
        ...
        private void compile_procedure_call_or_property_set(TextWriter target)
        {
            ...
            else
            {
                ...
                if (libentry == null)
                {
                    //s.ThrowParseError("Undefined command: " + cmdname, 1, 1);
        ...
        private Expression parse_atomic_expression()
        {
            ...
            else if (s.NextType == SymType.ID)
            {
                ...
                else
                {
                    ...
                    if (!variables.ContainsKey(varname))
                    {
                        //s.ThrowParseError("can not use variable " + var_or_object + " before first assignment", 1, 1);
        ...
        private void extractfinfo_statement(string currentsub)
        {
            ...
            {   // some atomic statement
                ...
                {   // variable assignment
                    ...
                    parse_special("=");
                    //Console.WriteLine("1 " + id + " " + s.NextType + " " + s.NextContent);
                    
                    //Console.WriteLine("2 " + id + " " + s.NextType + " " + s.NextContent);

                    extractfinfo_expression(currentsub);
                    //Console.WriteLine("3 " + id + " " + s.NextType + " " + s.NextContent);
        ...
        private void extractfinfo_atomic_expression(string currentsub)
        {
            ...
            {
                string var_or_object = parse_id();  // must be a variable name or an object (depends on the precence of a '.' afterwards)
                //Console.WriteLine(var_or_object);
                if (s.NextIsSPECIAL("."))
                ...
                {   // is variable use
                    //Expression e = parse_expression();
                    //Console.WriteLine("===>" + var_or_object + "<===>" + variables.Count + "<===");
                }
            }
            else
            {
        ...
        internal bool functionCouldCall(FunctionDefinition f1, FunctionDefinition f2)
        {
        ...
        internal FunctionDefinition getCurrentFunction()
        {
